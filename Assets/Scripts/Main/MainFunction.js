...
function Update () {
  ...
  if (isBuilding) {
    ...
    if (Physics.Raycast(ray, hit, 1000, placementLayerMask)) {
      if (lastHitPlane) {
        lastHitPlane.renderer.material = originalMat;
      }
      lastHitPlane = hit.collider.gameObject;
      originalMat = lastHitPlane.renderer.material;
      lastHitPlane.renderer.material = hoverMat;
    } else {
      if (lastHitPlane) {
        lastHitPlane.renderer.material = originalMat;
        lastHitPlane = null;
    ...
    if (lastHitBuilding) {
      if (isCombining) { // Combinate
        if (!(lastHitBuilding.GetComponent(Stats).combinatable)) { // Unable to combinate
          ...
          nowBuildingNumber--;
          lastHitBuilding.GetComponent(Stats).planeTaken.renderer.tag = "PlacementPlaneOpen";
      ...
      */
      if (lastHitPlane.tag == "PlacementPlaneOpen" && buildingIndex >= 0) {
        if (money >= allBuildings[buildingIndex].GetComponent(Stats).cost && nowBuildingNumber < maxBuildingNumber) {
          ...
          lastHitPlane.gameObject.renderer.enabled = false;
  ...
  if (isBuilding) {
    for (var i : int = 0; i < 4; ++i) {
      if (Input.GetKeyDown(KeyCode.Keypad1 + i) || Input.GetKeyUp(KeyCode.Alpha1 + i)) {
        ...
        if (buildingIndex != i) { // Prevent mutil click
          ShowMessage("請選擇放置區域");
          buildingIndex = i;
        }
      }
  ...
  */
    ...
    */
    ...
    for (i = 0; i < allBuildings.length; ++i) {
      if (GUI.Button(Rect(i * 150 + 75, 0, 135, 30), allBuildings[i].tag + " (" + (i + 1) + ")")) { // Hotkey
        audio.PlayOneShot(buttonSound);
        if (i != buildingIndex) { // Prevent mutil click
          ShowMessage("請選擇放置區域");
          buildingIndex = i;
        }
      }
  ...
  if (selectingBuilding) {
    ...
    if (GUI.Button(Rect(50, 155, 150, 25), "拆除(80%金錢)")) {
      ...
      nowBuildingNumber--;
      selectingBuilding.GetComponent(Stats).planeTaken.renderer.tag = "PlacementPlaneOpen";
  ...
  for (var plane : Transform in placementPlanesRoot) {
    if (isBuilding) {
      if (plane.tag == "PlacementPlaneOpen") { // Only show the empty plane
        plane.gameObject.renderer.enabled = isBuilding;
      }
    } else {
      plane.gameObject.renderer.enabled = isBuilding;
    }

...
function Update () {
  if (!movable) {
    ...
    if (camera.main.GetComponent(WaveHandler).restTime < 0) {
      camera.main.GetComponent(WaveHandler).ShowMessage("超級病菌出現");
      camera.main.GetComponent(WaveHandler).ShowMessage("想辦法抵擋超級病菌的攻勢");
      movable = true;
...
function Attack () {
  ...
  if (target.GetComponent(Stats).nowHP <= 0) {
    ...
    camera.main.GetComponent(MainFunction).nowBuildingNumber--;
  ...
  renderer.material = materialAttack;
  yield WaitForSeconds(0.5);
  renderer.material = materialGeneral;
}
